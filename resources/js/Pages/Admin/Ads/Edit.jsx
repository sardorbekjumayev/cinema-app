...
import { ChevronLeft, Upload, Save, Eye, EyeOff, Trash2 } from 'lucide-react';
import { useState } from 'react';

export default function Edit({ ad }) {
    const [preview, setPreview] = useState(`/storage/${ad.image}`);
    ...
    const submit = (e) => {
        e.preventDefault();
        // Since we are uploading a file, we use POST with _method PUT
        post(route('admin.ads.update', ad.id));
    };

    return (
        <AdminLayout>
            <Head title={`Admin - Tahrirlash: ${ad.title}`} />
            ...
            <div className="space-y-10">
                <div className="flex justify-between items-end">
                    <div>
                        ...
                        <h1 className="text-4xl font-black tracking-tighter uppercase italic">Reklamani <span className="text-indigo-500">Tahrirlash</span></h1>
                        <p className="text-white/40 font-medium mt-2 text-lg">Reklama tafsilotlarini va maqsadli auditoriyani yangilang.</p>
                    </div>
                </div>
