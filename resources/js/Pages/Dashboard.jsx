...
import { Search, Film, Star, Clock, Globe, Play } from 'lucide-react';
import { useState, useEffect } from 'react';
import AdBanner from '@/Components/Cinema/AdBanner';

export default function Dashboard({ auth, movies, categories, filters, ads }) {
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            if (search !== (filters.search || '')) {
                router.get(route('user.dashboard'), { search, category: activeCategory }, { preserveState: true, replace: true });
            }
        }, 300);
    ...
    const handleCategoryClick = (slug) => {
        ...
        router.get(route('user.dashboard'), { search, category: nextCategory }, { preserveState: true });
    };

    return (
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-indigo-500 selection:text-white">
            <Head title="Foydalanuvchi Paneli - Cinema" />
                        ...
                        <div className="flex items-center gap-4">
                                ...
                                <div className="flex items-center justify-end gap-2">
                                    {auth.user.subscription_expires_at && (
                                        <p className="text-[8px] font-black uppercase tracking-widest text-white/20">
                                            Muddati: {new Date(auth.user.subscription_expires_at).toLocaleDateString()}
                                        </p>
                                    )}
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 capitalize">{auth.user.status === 'premium' ? 'Premium' : 'Oddiy'} Hisob</p>
                                </div>
                            </div>
                            ...
                            {auth.user.role === 'admin' && (
                                <Link
                                    ...
                                    className="hidden md:flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all"
                                >
                                    Admin Panel
                                </Link>
                            )}

                            <Link href={route('profile.edit')} className="h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-500 border border-white/20 flex items-center justify-center font-black hover:scale-110 transition-transform">
                                {auth.user.name[0]}
                            </Link>
                            <Link
                                href={route('logout')}
                                method="post"
                                as="button"
                                className="text-xs font-black uppercase tracking-widest text-white/40 hover:text-red-500 transition-colors"
                            >
                                Chiqish
                            </Link>
                        </div>
                ...
                <div className="max-w-7xl mx-auto relative">
                    <div className="max-w-3xl">
                        <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-indigo-500/10 border border-indigo-500/20 mb-8">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">Xush kelibsiz, {auth.user.name}</span>
                        </div>
                        <h1 className="text-6xl md:text-8xl font-black tracking-tighter uppercase italic leading-[0.9]">
                            Shaxsiy <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-violet-400">Kutubxonangiz</span>
                        </h1>
                        <p className="mt-8 text-xl text-white/40 font-medium max-w-xl leading-relaxed">
                            Qolgan joyidan davom eting yoki kollektsiyangizdan yangi durdonalarni kashf eting.
                        </p>
                    </div>
